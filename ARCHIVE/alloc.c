...
struct alloc_result alloc(uint32_t size)
{
    ...
    if(size > MAX_POOL_SIZE) {
        return (struct alloc_result) {
            general_allocator.super.alloc(&general_allocator, size),
            false
        };
    }
    ...
    if(mem == NULL){
        ...
        if (mem == NULL) {
            return (struct alloc_result) {mem, -1};
        }
        return (struct alloc_result){mem, false};
    }

    return (struct alloc_result){mem, true};
}
